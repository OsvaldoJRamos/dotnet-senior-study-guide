...
For the application-level counterpart (`lock`, `Monitor`, `SemaphoreSlim`), see [Locks in Depth](../06-concurrency-and-parallelism/07-locks-in-depth.md). For isolation-level semantics, see [Transaction Isolation Levels](06-transaction-isolation-levels.md).
...
- **See [Locks in Depth](../06-concurrency-and-parallelism/07-locks-in-depth.md)** for the application-side companion (`lock`, `Monitor`, `SemaphoreSlim`, `Interlocked`).
