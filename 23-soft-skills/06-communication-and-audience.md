...
Code shows *what*. The doc exists because the *why* isn't in the code. Every architectural doc, ADR, and RFC should answer *why this, why not the alternatives, what does this cost us*. See [Design Docs, C4 and ADRs](../08-architecture-and-patterns/17-design-docs-c4-adr.md).
...
Pick the level of abstraction the audience needs and stay there. A diagram mixing users, HTTP endpoints, DB tables, **and** internal classes asks the reader to re-zoom on every line. Split it. C4 levels make this explicit (see [UML Diagrams](../08-architecture-and-patterns/19-uml-diagrams.md)).
...
- **C4 for system shape**, sequence for flows, state for lifecycles, ER for schemas. See [UML Diagrams](../08-architecture-and-patterns/19-uml-diagrams.md).
