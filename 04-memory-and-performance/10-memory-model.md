...
> This file focuses on the **conceptual model** — visibility, ordering, and the patterns built on top. For the BCL primitives that implement these guarantees (`lock`, `Monitor`, `Interlocked`, `Volatile`, `volatile`, `SemaphoreSlim`), see [Locks in Depth](../05-concurrency-and-parallelism/07-locks-in-depth.md).
